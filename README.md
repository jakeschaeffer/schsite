...
This site displays movies and TV shows you've watched, with two options for managing your data:

#### Option 1: Automatic Tracking with Trakt (Recommended)

**Trakt.tv** is a service designed for tracking what you watch. It automatically syncs your history and provides a great UI for managing your data.

1. **Create a Trakt Account**:
   - Sign up at [Trakt.tv](https://trakt.tv)
   - Mark movies and TV shows as watched (web or mobile app)
   - Optional: Enable scrobbling to auto-track from streaming services

2. **Get Trakt API Credentials**:
   - Go to [Trakt Applications](https://trakt.tv/oauth/applications)
   - Create a new application:
     - Name: "Personal Website" (or whatever you prefer)
     - Redirect URI: `urn:ietf:wg:oauth:2.0:oob` (for manual token)
     - Permissions: Can be set to minimal
   - Copy your **Client ID**

3. **Get an Access Token**:
   - Use [this tool](https://trakt.tv/oauth/authorize?response_type=code&client_id=YOUR_CLIENT_ID&redirect_uri=urn:ietf:wg:oauth:2.0:oob) (replace YOUR_CLIENT_ID)
   - Or use the [Trakt API docs](https://trakt.docs.apiary.io/#reference/authentication-oauth) to get a token
   - Copy your **Access Token**

4. **Configure Environment Variables**:
   - Copy `.example.env` to `.env`: `cp .example.env .env`
   - Add your Trakt credentials and TMDB API key:
     ```env
...
5. **Build & Deploy**:
   - Run `pnpm build` - your watch history will automatically load from Trakt!
   - Rebuild whenever you want to update your site with new watches

**Trakt Benefits**:
- ✅ Episode-level tracking with detailed progress
- ✅ Auto-sync with streaming services (scrobbling)
- ✅ Mobile apps for easy tracking
- ✅ No manual data entry in code
- ✅ Organized by year automatically

#### Option 2: Manual Entry

If you prefer not to use Trakt, you can manually maintain your watch history:

1. **Get a TMDB API Key** (required for poster images):
   - Sign up at [The Movie Database](https://www.themoviedb.org/)
   - Go to [API Settings](https://www.themoviedb.org/settings/api) to get an API key
   - Add to `.env`: `TMDB_API_KEY=your_api_key_here`

2. **Edit Watch History**:
   - Open `src/data/watch-history.ts`
   - Add entries to the `watchHistory` array:
     ```typescript
     {
       year: 2025,
       entries: [
         { tmdbId: 864, type: "movie", notes: "Cool Runnings", rating: "up" },
         { tmdbId: 95396, type: "tv", notes: "S2E1-3", rating: "up" },
       ]
     }
     ```
   - Find TMDB IDs by searching on [themoviedb.org](https://www.themoviedb.org)

**Note**: Data is fetched at build time, so rebuild (`pnpm build`) to see changes.
