...
This site includes integration with The Movie Database (TMDB) API to display movies and TV shows.

1. **Get a TMDB API Key**:
   - Sign up for a free account at [The Movie Database](https://www.themoviedb.org/)
   - Go to [API Settings](https://www.themoviedb.org/settings/api) to request an API key
   - Copy your API key
...
3. **Customize Movie & TV Show Lists**:
   - Edit `src/pages/movies.astro` to update the `movieIds` array with your preferred movie IDs
   - Edit `src/pages/shows.astro` to update the `showIds` array with your preferred TV show IDs
   - Find movie/show IDs by searching on [TMDB](https://www.themoviedb.org/) and checking the URL

**Note**: Movie and TV show data is fetched at build time, so you'll need to rebuild the site (`pnpm build`) whenever you want to update the displayed content.
